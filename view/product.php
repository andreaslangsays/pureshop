...
if($p->has_ve)
	...
	##############################################################################
		...
		$ve_price_string =  $currencies->format($p->ve_end_price) . " <span class='tx_15_30" . $col . "' style='bottom:1px;display:inline-block;position:relative;'>/</span> VE";
		...
		if($p->special)
		{
			echo "<span class='tx_stroke tx_13_20'>" . $currencies->display_price($p->ve_old_end_price) . " / VE</span><br />";
		}
...
<?php
	...
	?>
	<form id="prodConfForm"  action="<?php echo push_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $p->products_id . '&action=fast_buy') ?>" method="post" name="cart_quantity"  style="width:300px; position:relative;">
	<?php
		$p->get_special_tag();
		if($p->special)
		{
			echo "<span class='tx_stroke tx_13_20'>" . $currencies->display_price($p->price) . " / Stück</span><br />";
			
		}
	?>
		<span class='tx_25_30<?= $col ?>' ><?php echo $currencies->display_price($p->final_price); ?></span> 
