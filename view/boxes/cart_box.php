...
<?php if ($customer->login) { ?>
		...
		<?php
			...
			<?php
				...
				{
			?>
					<span class="bx_left tx_pink tx_13_20">
					<img style="vertical-align: middle; margin-right: 5px" src="images/assets/ico_discount_individual.png">Individueller Kundenbonus</span>
					<div class="clearfix"></div>
					<span class="bx_left tx_pink tx_13_20"><img style="vertical-align: middle;margin-left:34px; margin-right: 5px" src="images/push/icons/ico_discount_true-s.png"><?php echo $discount->customer_discount_table; ?>% Rabatt auf <?php echo $currencies->format($discount->private_rabatt_amount,1); ?> </span><span class="bx_right tx_pink tx_13_20"><?php echo $currencies->format( (-1 * $discount->private_rabatt_saving), 1) ?></span>
					<div class="clearfix"></div>
			<?php
				}
			...
			<?php
				...
				{
			?>
					<span class="bx_left tx_pink tx_13_20"><img style="vertical-align: middle; margin-right: 5px" src="images/push/icons/ico_discount_true-s.png"><?php echo $discount->coupon['name']; ?> Gutschein</span><span class="bx_right tx_pink tx_13_20"><?php echo $currencies->format( (-1 * $discount->coupon['reduction']), 1) ?></span>
					<div class="clearfix"></div>
			<?php
				}//EOF DISCOUNTS
			...
			<?php
				...
				{
					?><span class="tx_pink tx_13_20 tx_strong">Gratis:</span><br /><?php
					$gp = new product;
					foreach($discount->free_product_ids as $gratis)
					{
						$gp->load_product($gratis);
						$img = $gp->get_image('cartbox',30);
					?>
<!--					<span class="cart_t1 tx_13_20">
						<a href="<?= push_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $gp->products_id) ?>" ><?= push_image(DIR_WS_IMAGES . $img, $p->products_name) ?></a>
					</span>-->
					<span class="bx_left tx_pink tx_13_20" style="line-height:38px !important;"><a href="<?= push_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $gp->products_id) ?>" style="display:block;height:38px;width:38px;min-height:38px;min-width:38px;border:1px solid rgb(187,187,187);line-height:38px;float:left;text-align:center;margin-right:10px;"><?= push_image(DIR_WS_IMAGES . $img, $p->products_name,'','','style="vertical-align:middle;"') ?></a><?php echo $gp->products_name ?></span><span class="bx_right tx_pink tx_13_20" style="line-height:38px !important;">0,00 EUR</span>
					<div class="clearfix"></div>
					<?php
					}
				}
...
<?php if(isset($_SESSION['customer_id']) ){ 
	$anzeigen = $discount->get_next_discount_string();
	if($anzeigen <>'')
	{
?>
	<div id="rabatt_info" class="tx_pink tx_13_15" <?php echo ($_COOKIE['displayhint']== 'yes')? ' style="display:block;"':' style="display:block;"';?> >
	<?php
	...
	?>
	 <img class="btn_close" alt="" src="images/push/btn_schliessen30.png"/>
		...
		{
			?>
			<div id="display_rabatt" <?php echo ($_COOKIE['displayhint']== 'yes')?' style="display:none;"':' style="display:none;"';?>>
