...
*/
  ...
  class ot_giftwrap {
    ...
    function process() {
      ...
      if (push_not_null($order->info['giftwrap_method'])) {
        if (MODULE_ORDER_TOTAL_GIFTWRAP_TAX_CLASS > 0) {
          $giftwrap_tax = push_get_tax_rate(MODULE_ORDER_TOTAL_GIFTWRAP_TAX_CLASS);

          $order->info['tax'] += push_calculate_tax($order->info['giftwrap_cost'], $giftwrap_tax);
          $order->info['tax_groups']["{$giftwrap_tax}"] += push_calculate_tax($order->info['giftwrap_cost'], $giftwrap_tax);
          $order->info['total'] += push_calculate_tax($order->info['giftwrap_cost'], $giftwrap_tax);

          if (DISPLAY_PRICE_WITH_TAX == true) $order->info['giftwrap_cost'] += push_calculate_tax($order->info['giftwrap_cost'], $giftwrap_tax);
        }
			if((int)$order->info['giftwrap_cost'] == 0)
				$titel=$order->info['giftwrap_method'] ;
			else
				$titel= "Geschenkverpackung";
        $this->output[] = array('title' => $titel. ':',
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = push_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_ORDER_TOTAL_GIFTWRAP_STATUS'");
