...
global $PHP_SELF, $order, $currencies;
	...
	if ($od_amount > 0) {
		#Amended by EK for TerraNetwork 2007-02-26 to fix tax not correctly calculated bug (tax deducted twice if display tax is true)
...
global  $currencies, $language;
	$selection_string = '';
	// $selection_string .= '<form class="prefix_6 grid_10 coupon_form" method="post" style=""><div style="position:relative;overflow:hidden"><span class="tx_15_20" style="display: block; margin-bottom: 5px">Gutschein einlösen</span>'. push_draw_input_field('gv_redeem_code', '',' ') . '<input class="gradientgrey" type="submit" name="redeem" value="Einlösen" ><br><span class="tx_lightgrey tx_12_15" >' . TEXT_COUPON_EXPLAIN . "</span>";
	$selection_string .= '<div style="position:relative;overflow:hidden"><span class="tx_15_20" style="display: block; margin-bottom: 5px">Gutschein einlösen</span>'. push_draw_input_field('gv_redeem_code', '',' ') . '<input class="gradientgrey" type="submit" name="redeem" value="Einlösen" ><br><span class="tx_lightgrey tx_12_15" >' . TEXT_COUPON_EXPLAIN . "</span>";
...
global $_POST, $currencies, $cc_id, $couponCode;
	if (push_session_is_registered('couponCode')) {

// get some info from the coupon table
	$coupon_query=push_db_query("select coupon_id, coupon_amount, coupon_type, coupon_minimum_order,uses_per_coupon, uses_per_user, restrict_to_products,restrict_to_categories from " . TABLE_COUPONS . " where coupon_code='".$couponCode."' and coupon_active='Y'");
	$coupon_result=push_db_fetch_array($coupon_query);

	if ($coupon_result['coupon_type'] != 'G') {

		if (push_db_num_rows($coupon_query)==0) {
			push_session_unregister('couponCode');
			push_redirect(push_href_link(FILENAME_CHECKOUT_CONFIRMATION, 'error_message=' . urlencode(ERROR_NO_INVALID_REDEEM_COUPON), 'SSL'));
		}

		$date_query=push_db_query("select coupon_start_date from " . TABLE_COUPONS . " where coupon_start_date <= now() and coupon_code='".$couponCode."'");

		if (push_db_num_rows($date_query)==0) {
			push_session_unregister('couponCode');
			push_redirect(push_href_link(FILENAME_CHECKOUT_CONFIRMATION, 'error_message=' . urlencode(ERROR_INVALID_STARTDATE_COUPON), 'SSL'));
		}

		$date_query=push_db_query("select coupon_expire_date from " . TABLE_COUPONS . " where coupon_expire_date >= now() and coupon_code='".$couponCode."'");

    if (push_db_num_rows($date_query)==0) {
		push_session_unregister('couponCode');
			push_redirect(push_href_link(FILENAME_CHECKOUT_CONFIRMATION, 'error_message=' . urlencode(ERROR_INVALID_FINISDATE_COUPON), 'SSL'));
		}

		$coupon_count = push_db_query("select coupon_id from " . TABLE_COUPON_REDEEM_TRACK . " where coupon_id = '" . $coupon_result['coupon_id']."'");
		$coupon_count_customer = push_db_query("select coupon_id from " . TABLE_COUPON_REDEEM_TRACK . " where coupon_id = '" . $coupon_result['coupon_id']."' and customer_id = '" . $_SESSION['customer_id'] . "'");

		if (push_db_num_rows($coupon_count)>=$coupon_result['uses_per_coupon'] && $coupon_result['uses_per_coupon'] > 0) {
			push_session_unregister('couponCode');
			 push_redirect(push_href_link(FILENAME_CHECKOUT_CONFIRMATION, 'error_message=' . urlencode(ERROR_INVALID_USES_COUPON . $coupon_result['uses_per_coupon'] .  TIMES ), 'SSL'));
		}

		if (push_db_num_rows($coupon_count_customer)>=$coupon_result['uses_per_user'] && $coupon_result['uses_per_user'] > 0) {
			push_session_unregister('couponCode');
			push_redirect(push_href_link(FILENAME_CHECKOUT_CONFIRMATION, 'error_message=' . urlencode(ERROR_INVALID_USES_USER_COUPON . $coupon_result['uses_per_user'] . TIMES ), 'SSL'));
		}
		...
		if (!push_session_is_registered('cc_id')) push_session_register('cc_id'); //Fred - this was commented out before
	...
	if ($_POST['submit_redeem_coupon_x'] && !$couponCode) {
		push_session_unregister('couponCode');
		 push_redirect(push_href_link(FILENAME_CHECKOUT_CONFIRMATION, 'error_message=' . urlencode(ERROR_NO_REDEEM_CODE), 'SSL'));
	}}
}
...
//$cc_id = $_SESSION['cc_id']; //Fred commented out, do not use $_SESSION[] due to backward comp. Reference the global var instead.
	$od_amount = 0;
	if (isset($cc_id) ) {
		$coupon_query = push_db_query("select coupon_code from " . TABLE_COUPONS . " where coupon_id = '" . $cc_id . "'");
		if (push_db_num_rows($coupon_query) !=0 ) {
			$coupon_result = push_db_fetch_array($coupon_query);
			$this->coupon_code = $coupon_result['coupon_code'];
			$coupon_get = push_db_query("select coupon_amount, coupon_minimum_order, restrict_to_products, restrict_to_categories, coupon_type from " . TABLE_COUPONS ." where coupon_code = '". $coupon_result['coupon_code'] . "'");
			$get_result = push_db_fetch_array($coupon_get);
					...
					for ($i=0; $i<sizeof($order->products); $i++) {
						if ($get_result['restrict_to_products']) {
							...
							for ($ii = 0; $ii < count($pr_ids); $ii++) {
								if ($pr_ids[$ii] == push_get_prid($order->products[$i]['id'])) {
								...
								for ($i=0; $i<sizeof($order->products); $i++) {
									$my_path = push_get_product_path(push_get_prid($order->products[$i]['id']));
		...
		if ($get_result['coupon_type'] != 'S') {
			...
			if ($get_result['restrict_to_products'] || $get_result['restrict_to_categories']) {
				...
				$valid_product = false;
					...
					$cc_query = push_db_query("select products_tax_class_id from " . TABLE_PRODUCTS . " where products_id = '" . $t_prid . "'");
					$cc_result = push_db_fetch_array($cc_query);
					if ($get_result['restrict_to_products']) {
						$pr_ids = split("[,]", $get_result['restrict_to_products']);
					...
					if ($get_result['restrict_to_categories']) {
						...
						for ($c = 0; $c < sizeof($cat_ids); $c++) {
							$cat_query = push_db_query("select products_id from products_to_categories where products_id = '" . $products_id . "' and categories_id = '" . $cat_ids[$i] . "'");
							if (push_db_num_rows($cat_query) !=0 ) $valid_product = true;
						}
					}
					if ($valid_product) {
				...
				if (sizeof($valid_array) > 0) { // if ($valid_product) {
					...
					if ($method=='Credit Note') {
						...
						for ($p=0; $p<sizeof($valid_array); $p++) {
							$tax_rate = push_get_tax_rate($valid_array[$p]['products_tax_class'], $order->delivery['country']['id'], $order->delivery['zone_id']);
							$tax_desc = push_get_tax_description($valid_array[$p]['products_tax_class'], $order->delivery['country']['id'], $order->delivery['zone_id']);
							if ($tax_rate > 0) {
...
global $insert_id, $REMOTE_ADDR, $cc_id;
	//$cc_id = $_SESSION['cc_id']; //Fred commented out, do not use $_SESSION[] due to backward comp. Reference the global var instead.
	if ($this->deduction !=0) {
		push_db_query("insert into " . TABLE_COUPON_REDEEM_TRACK . " (coupon_id, redeem_date, redeem_ip, customer_id, order_id) values ('" . $cc_id . "', now(), '" . $REMOTE_ADDR . "', '" . $_SESSION['customer_id'] . "', '" . $insert_id . "')");
	}
	push_session_unregister('cc_id');
}

function get_order_total() {
global $order, $cart, $cc_id;
	//$cc_id = $_SESSION['cc_id']; //Fred commented out, do not use $_SESSION[] due to backward comp. Reference the global var instead.
			...
			for ($i = 0; $i < count($cat_ids); $i++) {
				if (is_array($this->contents)) {
					...
					while (list($products_id, ) = each($this->contents)) {
						$cat_query = push_db_query("select products_id from products_to_categories where products_id = '" . $products_id . "' and categories_id = '" . $cat_ids[$i] . "'");
						if (push_db_num_rows($cat_query) !=0 ) {
							$in_cat = true;
		...
		if ($get_result['restrict_to_products']) {
			...
			for ($i = 0; $i < sizeof($pr_ids); $i++) {
				for ($ii = 1; $ii<=sizeof($products_array); $ii++) {
					if (push_get_prid($products_array[$ii-1]['id']) == $pr_ids[$i]) {
						$in_cart=true;
...
global $cart, $order;
	$products_id = push_get_prid($product_id);
	// products price
	$qty = $cart->contents[$product_id]['qty'];
	$product_query = push_db_query("select products_id, products_price, products_tax_class_id, products_weight from " . TABLE_PRODUCTS . " where products_id='" . $product_id . "'");
	if ($product = push_db_fetch_array($product_query)) {
		$prid = $product['products_id'];
		$products_tax = push_get_tax_rate($product['products_tax_class_id']);
		$products_price = $product['products_price'];
		$specials_query = push_db_query("select specials_new_products_price from " . TABLE_SPECIALS . " where products_id = '" . $prid . "' and status = '1'");
		if (push_db_num_rows ($specials_query)) {
			$specials = push_db_fetch_array($specials_query);
			$products_price = $specials['specials_new_products_price'];
		}
		if ($this->include_tax == 'true') {
			$total_price += ($products_price + push_calculate_tax($products_price, $products_tax)) * $qty;
//			echo("total price = " . $total_price . " products_price = " . $products_price . " products_tax = " . $products_tax . "<br>");
		...
		if (isset($cart->contents[$product_id]['attributes'])) {
			...
			while (list($option, $value) = each($cart->contents[$product_id]['attributes'])) {
				$attribute_price_query = push_db_query("select options_values_price, price_prefix from " . TABLE_PRODUCTS_ATTRIBUTES . " where products_id = '" . $prid . "' and options_id = '" . $option . "' and options_values_id = '" . $value . "'");
				$attribute_price = push_db_fetch_array($attribute_price_query);
				if ($attribute_price['price_prefix'] == '+') {
					if ($this->include_tax == 'true') {
						$total_price += $qty * ($attribute_price['options_values_price'] + push_calculate_tax($attribute_price['options_values_price'], $products_tax));
					} else {
...
function check() {
	if (!isset($this->check)) {
		$check_query = push_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_ORDER_TOTAL_COUPON_STATUS'");
...
function install() {
	push_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Display Total', 'MODULE_ORDER_TOTAL_COUPON_STATUS', 'true', 'Do you want to display the Discount Coupon value?', '6', '1','push_cfg_select_option(array(\'true\', \'false\'), ', now())");
	push_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_ORDER_TOTAL_COUPON_SORT_ORDER', '9', 'Sort order of display.', '6', '2', now())");
	push_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function ,date_added) values ('Include Shipping', 'MODULE_ORDER_TOTAL_COUPON_INC_SHIPPING', 'true', 'Include Shipping in calculation', '6', '5', 'push_cfg_select_option(array(\'true\', \'false\'), ', now())");
	push_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function ,date_added) values ('Include Tax', 'MODULE_ORDER_TOTAL_COUPON_INC_TAX', 'true', 'Include Tax in calculation.', '6', '6','push_cfg_select_option(array(\'true\', \'false\'), ', now())");
