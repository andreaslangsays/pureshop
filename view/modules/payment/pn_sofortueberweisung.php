...
class pn_sofortueberweisung {
	...
	function selection() {
		global $scart_pn_sofortueberweisung_ID, $language;

		if (push_session_is_registered('cart_pn_sofortueberweisung_ID')) {
			$order_id = substr($scart_pn_sofortueberweisung_ID, strpos($scart_pn_sofortueberweisung_ID, '-')+1);
	...
	function confirmation() {
		global $cartID, $scart_pn_sofortueberweisung_ID,  $languages_id, $language, $order, $order_total_modules;
		global $redirect;
		if($redirect !== true)
			return false;
		...
		if (push_session_is_registered('cart_pn_sofortueberweisung_ID')) {
			...
			if ( ($curr['currency'] != $order->info['currency']) || ($cartID != substr($scart_pn_sofortueberweisung_ID, 0, strlen($cartID))) ) {
				$check_query = push_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . $order_id . '" limit 1');
		...
		if ($insert_order == true) {
			...
			$scart_pn_sofortueberweisung_ID = $cartID . '-' . $insert_id;
	...
	function process_button() {
		global $order, $cart,  $currencies, $currency, $scart_pn_sofortueberweisung_ID, $redirect;
		...
		$reason_1 = str_replace('{{customer_id}}', $_SESSION['customer_id'], $reason_1);
		...
		$reason_2 = str_replace('{{customer_id}}', $_SESSION['customer_id'], $reason_2);
		...
		$user_variable_1 = push_output_string($_SESSION['customer_id']);
	...
	function before_process() {
		global $order, $order_totals, $sendto, $billto, $languages_id, $payment, $currencies, $currency, $cart, $scart_pn_sofortueberweisung_ID;
		global $$payment;
	...
	//------insert customer choosen option eof ----
		...
		if ($order->content_type != 'virtual') {
			$text = str_replace('{{DELIVERY_ADRESS}}', push_address_label($_SESSION['customer_id'], $sendto, 0, '', "\n"), $text);
		}
		...
		$text = str_replace('{{BILLING_ADRESS}}', push_address_label($_SESSION['customer_id'], $billto, 0, '', "\n"), $text);  
