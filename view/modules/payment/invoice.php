...
// class methods
    function update_status() {
	  ...
	  if (MODULE_PAYMENT_INVOICE_FROM_ORDER > 0){
			$test_query = push_db_query("select count(*) as total from " . TABLE_ORDERS . " where customers_id='" . $_SESSION['customer_id'] . "' AND orders_status = 3" );
	  ...
	  // start änderung
	  $test_query = push_db_query("select count(*) as total from " . TABLE_ORDERS . " where customers_id='".$_SESSION['customer_id']."' AND orders_status>'" . MODULE_PAYMENT_INVOICE_ORDER_STATUS_ID . "'");
	  $result = push_db_fetch_array($test_query);

	  $inv_query = push_db_query("select customers_invoice from " . TABLE_CUSTOMERS . " where customers_id='".$_SESSION['customer_id']."'");
