...
class germanlastschrift {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_GERMANRT_ZONE > 0) ) {
    	...
    	if (MODULE_PAYMENT_GERMANRT_ENABLE_AFTER == 'true'){
      	$test_query = push_db_query("select count(*) as total from " . TABLE_ORDERS . " where customers_id='" . $_SESSION['customer_id'] . "' AND orders_status = " . MODULE_PAYMENT_GERMANRT_ENABLE_AFTER_ORDER_STATUS );
    ...
    function selection() {
      ...
      $bankdata_query = push_db_query("select * from " . TABLE_CUSTOMERS . " where customers_id='" . $_SESSION['customer_id'] . "'");
 ...
 //       if ($_POST['banktransfer_fax'] == false) {
   ...
   //             }
    ...
    function after_process() {
      global $insert_id, $gbt_number, $gbt_array,  $sql_bankdata_array;
	...
	if($_SESSION['customer_id']=='36767'){
		...
		die();
  		...
  		push_db_perform(TABLE_CUSTOMERS, $sql_bankdata_array, 'update', "customers_id = '" . (int)$_SESSION['customer_id'] . "'");
