...
<div class="grid_16" style="height:10px;border-bottom:1px solid #ccc;margin-bottom:15px;" ></div>
	...
	<?php 	} else {
				...
				<?php
					...
					{
				?>
					<div class="prefix_6 grid_4 bx_left tx_darkpink tx_13_20">Versandkosten </div><div class="grid_2 suffix_4 tx_darkpink bx_left tx_13_20"><?=  $currencies->format($discount->shipping_savings,1)?></div>
					<div class="clearfix"></div>
				...
				<?php
					...
					{
				?>
						<div class="prefix_6 grid_4 bx_left tx_darkpink tx_13_20"><?php echo $discount->actual_discount_procent; ?>% Rabatt auf <?php echo $currencies->format($discount->discount_amount,1); ?> </div><div class="grid_6 tx_right tx_13_20 tx_darkpink"><?php echo $currencies->format( (-1 * $discount->general_savings), 1) ?></div>
						<div class="clearfix"></div>
				<?php
					}
					...
					{
				?>
						<div class="prefix_6 grid_4 bx_left tx_darkpink tx_13_20"><?php echo ONLINE_RABATT_PROCENT; ?>% Onlinerabatt </div><div class="grid_6 tx_right tx_13_20 tx_darkpink"><?php echo $currencies->format( (-1 * $discount->online_rabatt), 1) ?></div>
						<div class="clearfix"></div>
				<?php
					}
				}
		...
		$(document).ready(function(){
			...
			$('.cartpricebox b').click(function(ev){
					...
					if($(this).text() == '+'  && (parseInt(inpt.val()) < inpt.attr("data-max")))
					{
						...
						window.location.href="<?= push_href_link(basename($_SERVER['SCRIPT_NAME']), push_get_all_get_params(array('action')) . 'action=updateproduct', 'SSL')?>&pid=" + hid.val() + "&qty=" + inpt.val();
					}
			...
			//13 maps to the enter key		
				var hid = $(this).parent().children("input[type='hidden']");
				if (e.keyCode == 13) {
					window.location.href="<?= push_href_link(basename($_SERVER['SCRIPT_NAME']), push_get_all_get_params(array('action')) . 'action=updateproduct', 'SSL')?>&pid=" + hid.val() + "&qty=" + $(this).val();
				}
			});
