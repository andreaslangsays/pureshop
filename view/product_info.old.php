...
?>
<div id="productdetail"  itemscope itemtype="http://data-vocabulary.org/Product">
...
<?php
echo '		<span></span><a href="' . push_href_link(DIR_WS_IMAGES . 'fullsize/' . $product->products_image) . '" class="imgpop"><img itemprop="image" src="' .DIR_WS_IMAGES .$product->get_image('detail',320) . '" alt="' . $product->products_name . ' Produktbild" title="' . $product->products_name . '"/></a>';
?>

		</div>
	<?php echo $product->get_infographics('d');?>
	</div>
<!--Image end-->
</div>
...
<?php
if($product->products_quantity > 0 && $customer->login)
{
	$col='';
	if($product->special)
	{
...
<?php
	...
	?>
		...
		if ( ($product->final_price > 0) && (!in_array($product->products_availability_id , $ired)) )
		{
...
<a  itemprop="brand" href="<?php echo ((false) ? push_href_link(FILENAME_MANUFACTURERS, 'manufacturers_id=' . $product->manufacturers_id) : '' ) ;?>" ><?= $product->manufacturers_name ?></a>
</div>
<?php
	}
