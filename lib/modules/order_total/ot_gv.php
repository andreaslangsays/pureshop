...
// if there has been something before us then
    ...
    function use_credit_amount() {
		global $cot_gv, $currencies, $order, $customer_id, $insert_id, $REMOTE_ADDR;
//      $_SESSION['cot_gv'] = false;     // old code - Strider
      $cot_gv = false;
      if ($this->selection_test()) {
			$gv_query = push_db_query("select amount from " . TABLE_COUPON_GV_CUSTOMER . " where customer_id = '" . $customer_id . "'");
...
//        $gv_order_amount += 0.001;
        ...
        if (MODULE_ORDER_TOTAL_GV_QUEUE == 'false') {
          ...
          if ($customer_gv) {
            $gv_update=push_db_query("update " . TABLE_COUPON_GV_CUSTOMER . " set amount = '" . $total_gv_amount . "' where customer_id = '" . $customer_id . "'");
          } else {
            $gv_insert=push_db_query("insert into " . TABLE_COUPON_GV_CUSTOMER . " (customer_id, amount) values ('" . $customer_id . "', '" . $total_gv_amount . "')");
          }
        } else {
         // GV_QUEUE is true - so queue the gv for release by store owner
          $gv_insert=push_db_query("insert into " . TABLE_COUPON_GV_QUEUE . " (customer_id, order_id, amount, date_created, ipaddr) values ('" . $customer_id . "', '" . $insert_id . "', '" . $gv_order_amount . "', NOW(), '" . $REMOTE_ADDR . "')");
        }
      }
    }
    ...
    function apply_credit() {
      ...
      if (push_session_is_registered('cot_gv')) {
        ...
        $gv_update = push_db_query("update " . TABLE_COUPON_GV_CUSTOMER . " set amount = '" . $gv_amount . "' where customer_id = '" . $customer_id . "'");
      }
      return $gv_payment_amount;
    }
    ...
    function collect_posts() {
      ...
      if ($_POST['gv_redeem_code']) {
        ...
        if ($gv_result['coupon_type'] == 'G') {
          ...
          $gv_amount_query=push_db_query("select amount from " . TABLE_COUPON_GV_CUSTOMER . " where customer_id = '" . $customer_id . "'");
          $customer_gv = false;
          ...
          $gv_redeem = push_db_query("insert into  " . TABLE_COUPON_REDEEM_TRACK . " (coupon_id, customer_id, redeem_date, redeem_ip) values ('" . $gv_result['coupon_id'] . "', '" . $customer_id . "', now(),'" . $REMOTE_ADDR . "')");
          if ($customer_gv) {
