...
*/
  ...
  class ot_freeproducts {
    ...
    function credit_selection() {
		global $customer_id, $order, $currencies, $language, $cart;
		...
		$this->get_customers_fpc($customer_id);
		...
		$sq=push_db_query("SELECT fpcc_products_id, COUNT(fpcc_products_id) AS quantity FROM freeproductscustomers WHERE fpcc_customers_id = '" . $customer_id . "' AND fpcc_status = 'pending' GROUP BY fpcc_products_id  ");
		...
		while($cc = push_db_fetch_array($sq))
		{
			if(array_key_exists($cc['fpcc_products_id'],  $this->products) )
			{
				...
				elseif($this->products[$cc['fpcc_products_id']]['qty'] < $cc['quantity'] )
				{
					...
					for($i; $i > 1; $i--){
						push_db_query("DELETE FROM freeproductscustomers WHERE fpcc_products_id='" . $cc['fpcc_products_id'] . "' AND fpcc_customers_id = '" . $customer_id . "' AND fpcc_status='pending' LIMIT 1;");
					}
				}
			}
			else
			{
				push_db_query("DELETE FROM freeproductscustomers WHERE fpcc_products_id='" . $cc['fpcc_products_id'] . "' AND fpcc_customers_id = '" . $customer_id . "' AND fpcc_status='pending';");
			}
 		}
	...
	function get_customers_fpc(){
		global $customer_id;
		$gutscheine='';
		$cq="SELECT * FROM freeproductscustomers WHERE fpcc_customers_id='" . (int)$customer_id . "' AND fpcc_status='pending'";
 ...
 */
	function update_credit_account() {
		global $order, $customer_id, $insert_id, $REMOTE_ADDR, $cart;
		$this->get_customers_fpc();
		foreach($this->customer_codes as $the_code)
		{
		//findout if product still inside order
			$cc= push_db_fetch_array(push_db_query("SELECT * FROM freeproductscustomers WHERE  fpcc_customers_id  = '" . $customer_id . "' AND BINARY fpcc_code = '" . $the_code . "'; "));
			if(array_key_exists($cc['fpcc_products_id'],  $this->products) )
			{
				//1st inside the customers table:
				push_db_query("UPDATE freeproductscustomers SET fpcc_status = 'redeemed' ,  fpcc_redeem_date = NOW()  WHERE fpcc_code = '" . $the_code . "' AND fpcc_customers_id   = '" . $customer_id . "';");
				...
				{
					push_db_query("UPDATE freeproductscode SET fpc_redeem = 'true',  fpc_redeem_ip= '" . $REMOTE_ADDR . "', fpc_redeem_customers_id= '" . $customer_id . "', fpc_redeem_date = NOW() WHERE BINARY fpc_code = '" . $the_code . "'");
				}
			}
		}
	/**/
	...
	function apply_credit() {
		global $order, $customer_id, $coupon_no, $cot_freeproducts, $cart;
		/* TEST * /
...
//FIRST CALLED METHOD in confirmation
	function collect_posts() {
		global $currencies, $_POST, $customer_id, $coupon_no, $REMOTE_ADDR, $cart, $code_invalid, $order;
		if(isset($_POST['gv_redeem_code'])){
			...
			foreach($holdc as $co){	//jeden Code einzeln durchgehen
				...
				$test="SELECT DISTINCT * FROM freeproductscode JOIN freeproductsentry ON
					...
					AND (fpc_redeem_customers_id='' OR fpc_redeem_customers_id='" . $customer_id . "')
					AND fpe_active='true' 
					AND fpe_start < NOW()
					AND NOW() < fpe_end";
   ...
   function use_credit_amount() {
		global $cot_freeproducts, $currencies, $cart, $order, $customer_id, $insert_id, $REMOTE_ADDR;
/* TEST * /
//      $_SESSION['cot_freeproducts'] = false;     // old code - Strider
      $cot_freeproducts = false;
      if ($this->selection_test()) {
			$gv_query = push_db_query("select amount from " . TABLE_COUPON_FREEPRODUCTS_CUSTOMER . " where customer_id = '" . $customer_id . "'");
