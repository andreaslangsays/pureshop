...
// class methods
	...
	function confirmation() {
		global $cartID, $cart_TELECASH_CONNECT_ID, $customer_id, $languages_id, $order, $order_total_modules;
	...
	// PandA.nl: register_globals fix
		...
		if (push_session_is_registered('cart_TELECASH_CONNECT_ID')) {
			$order_id = substr($cart_TELECASH_CONNECT_ID, strpos($cart_TELECASH_CONNECT_ID, '-')+1);
			$curr_check = push_db_query("select currency from " . TABLE_ORDERS . " where orders_id = '" . $order_id . "'");
			$curr = push_db_fetch_array($curr_check);

	//if ( ($curr['currency'] != $order->info['currency']) || ($cartID != substr($cart_TELECASH_CONNECT_ID, 0, strlen($cartID))) ) {
	...
	function process_button() {
		global $customer_id, $order, $languages_id, $currencies, $currency, $cart_TELECASH_CONNECT_ID, $shipping;
...
*/
		...
		$parameters['invoicenumber'] = substr($cart_TELECASH_CONNECT_ID, strpos($cart_TELECASH_CONNECT_ID, '-')+1);
