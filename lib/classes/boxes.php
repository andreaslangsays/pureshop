...
// class constructor
    function tableBox($contents, $direct_output = false) {
      $tableBox_string = '<table border="' . push_output_string($this->table_border) . '" width="' . push_output_string($this->table_width) . '" cellspacing="' . push_output_string($this->table_cellspacing) . '" cellpadding="' . push_output_string($this->table_cellpadding) . '"';
      if (push_not_null($this->table_parameters)) $tableBox_string .= ' ' . $this->table_parameters;
      $tableBox_string .= '>' . "\n";

      for ($i=0, $n=sizeof($contents); $i<$n; $i++) {
        if (isset($contents[$i]['form']) && push_not_null($contents[$i]['form'])) $tableBox_string .= $contents[$i]['form'] . "\n";
        $tableBox_string .= '  <tr';
        if (push_not_null($this->table_row_parameters)) $tableBox_string .= ' ' . $this->table_row_parameters;
        if (isset($contents[$i]['params']) && push_not_null($contents[$i]['params'])) $tableBox_string .= ' ' . $contents[$i]['params'];
        $tableBox_string .= '>' . "\n";
...
// EOF Products Description Hack
            if (isset($contents[$i][$x]['text']) && push_not_null($contents[$i][$x]['text'])) {
              $tableBox_string .= '    <td';
              if (isset($contents[$i][$x]['align']) && push_not_null($contents[$i][$x]['align'])) $tableBox_string .= ' align="' . push_output_string($contents[$i][$x]['align']) . '"';
              if (isset($contents[$i][$x]['valign']) && push_not_null($contents[$i][$x]['valign'])) $tableBox_string .= ' valign="' . $contents[$i][$x]['valign'] . '"'; // Added in v4.4
              if (isset($contents[$i][$x]['params']) && push_not_null($contents[$i][$x]['params'])) {
                $tableBox_string .= ' ' . $contents[$i][$x]['params'];
              } elseif (push_not_null($this->table_data_parameters)) {
                $tableBox_string .= ' ' . $this->table_data_parameters;
              }
              $tableBox_string .= '>';
              if (isset($contents[$i][$x]['form']) && push_not_null($contents[$i][$x]['form'])) $tableBox_string .= $contents[$i][$x]['form'];
              $tableBox_string .= $contents[$i][$x]['text'];
              if (isset($contents[$i][$x]['form']) && push_not_null($contents[$i][$x]['form'])) $tableBox_string .= '</form>';
              $tableBox_string .= '</td>' . "\n";
            }
          }
        } else {
          $tableBox_string .= '    <td';
          if (isset($contents[$i]['align']) && push_not_null($contents[$i]['align'])) $tableBox_string .= ' align="' . push_output_string($contents[$i]['align']) . '"';
          if (isset($contents[$i]['params']) && push_not_null($contents[$i]['params'])) {
        ...
        $tableBox_string .= '  </tr>' . "\n";
        if (isset($contents[$i]['form']) && push_not_null($contents[$i]['form'])) $tableBox_string .= '</form>' . "\n";
      }
  ...
  class infoBox extends tableBox {
    ...
    //$info_box_contents[] = array('text' => $this->infoBoxContents($contents, $contents[0]['type']));
    ...
    function infoBoxContents($contents) {
      ...
      $info_box_contents = array();
      $info_box_contents[] = array(array('text' => push_draw_separator('pixel_trans.gif', '100%', '1')));
      for ($i=0, $n=sizeof($contents); $i<$n; $i++) {
  ...
  class infoBoxHeading extends tableBox {
    function infoBoxHeading($contents, $left_corner = true, $right_corner = true, $right_arrow = false) {
      ...
      if ($left_corner == true) {
        $left_corner = push_image(DIR_WS_IMAGES . 'infobox/corner_left.gif');
      } else {
        $left_corner = push_image(DIR_WS_IMAGES . 'infobox/corner_right_left.gif');
      }
      if ($right_arrow == true) {
        $right_arrow = '<a href="' . $right_arrow . '">' . push_image(DIR_WS_IMAGES . 'infobox/arrow_right.gif', ICON_ARROW_RIGHT) . '</a>';
      } else {
        $right_arrow = '';
      }
      if ($right_corner == true) {
        $right_corner = $right_arrow . push_image(DIR_WS_IMAGES . 'infobox/corner_right.gif');
      } else {
        $right_corner = $right_arrow . push_draw_separator('pixel_trans.gif', '11', '14');
      }
  ...
  class contentBoxHeading extends tableBox {
    function contentBoxHeading($contents) {
        ...
        $info_box_contents = array();
        $info_box_contents[] = array(array('params' => 'height="14" class="infoBoxHeading"',
                                           'text' => push_image(DIR_WS_IMAGES . 'infobox/corner_left.gif')),
                                     array('params' => 'height="14" class="infoBoxHeading" width="100%"',
                                           'text' => $contents[0]['text']),
                                     array('params' => 'height="14" class="infoBoxHeading"',
                                           'text' => push_image(DIR_WS_IMAGES . 'infobox/corner_right_left.gif')));
        $this->tableBox($info_box_contents, true);
	    }
  	  // END STS
