<?php
/*
  $Idä: order_total.php,v 1.4 2003/02/11 00:04:53 hpdl Exp $
...
// class constructor
    function order_total() {
      ...
      if (defined('MODULE_ORDER_TOTAL_INSTALLED') && push_not_null(MODULE_ORDER_TOTAL_INSTALLED)) {
      ...
      if (is_array($this->modules)) {
        ...
        while (list(, $value) = each($this->modules)) {
          ...
          if ($GLOBALS[$class]->enabled) {
			...
			//$size= sizeof($GLOBALS[$class]->output);;
            for ($i=0; $i<$size; $i++) {
				if(! strpos($GLOBALS[$class]->output[$i]['title'],"(nein.)")){
				  $output_string .= '				<div class="grid_7">' . str_replace('**','', strip_tags($GLOBALS[$class]->output[$i]['title'],'<b>')) .'</div>' . "\n" .
                                	'				<div class="grid_3 suffix_6" style="text-align:right;">&nbsp;' . $GLOBALS[$class]->output[$i]['text'] . '</div>' . "\n" .
									'				<div class="grid_16" style="margin:8px 0 0 0;"></div>' . "\n"  ;
					}
            }
          }
        }
	...
	global $freeproducts;
      ...
      if (MODULE_ORDER_TOTAL_INSTALLED) {
       if(count($freeproducts)>0){
		$header_string .='<div style="margin:10px;">Eingelöster Gutschein: <br>';
			foreach($freeproducts as $fpr){
				$header_string .= $fpr . '<br>';
			}
		$header_string .= '</div>';	
		}
//  	 $header_string .= ' <table border="0" width="100%" cellspacing="0" cellpadding="2">' ."\n";
//       $header_string .= '       <tr><td width="10">' .  push_draw_separator('pixel_trans.gif', '10', '1') .'</td>' . "\n";
//       $header_string .= '           <td colspan="2"><table border="0" width="100%" cellspacing="0" cellpadding="2">' . "\n";
        $close_string   = '                         <!-- close -->';
//        $close_string  .= '<td width="10">' .  push_draw_separator('pixel_trans.gif', '10', '1') . '</td>';
//        $close_string  .= '</tr></table></td></tr></table></td>';
//        $close_string  .= '<tr><td width="100%">' .  push_draw_separator('pixel_trans.gif', '100%', '10') . '</td></tr>';
        reset($this->modules);
        $output_string = '';
        while (list(, $value) = each($this->modules)) {
          $class = substr($value, 0, strrpos($value, '.'));
          if ($GLOBALS[$class]->enabled && $GLOBALS[$class]->credit_class) {
            ...
            if ( ($use_credit_string !='' ) || ($selection_string != '') ) {
              $output_string = '';
			  $close_string = '' . $use_credit_string. '';
      ...
      if (MODULE_ORDER_TOTAL_INSTALLED) {
        ...
        while (list(, $value) = each($this->modules)) {
          ...
          if ( ($GLOBALS[$class]->enabled && $GLOBALS[$class]->credit_class) ) {
            ...
            if ($_POST[$post_var]) {
 							if (!push_session_is_registered($post_var)) push_session_register($post_var);
    ...
    function pre_confirmation_check() {
      ...
      if (MODULE_ORDER_TOTAL_INSTALLED) {
        ...
        if ($order->info['total'] - $total_deductions <= 0 ) {
					if(!push_session_is_registered('credit_covers')) push_session_register('credit_covers');
          $credit_covers = true;
        }
					...
					if(push_session_is_registered('credit_covers')) push_session_unregister('credit_covers');
				}
      }
    }
    ...
    function clear_posts() {
      ...
      if (MODULE_ORDER_TOTAL_INSTALLED) {
        ...
        while (list(, $value) = each($this->modules)) {
          ...
          if ( ($GLOBALS[$class]->enabled && $GLOBALS[$class]->credit_class) ) {
            $post_var = 'c' . $GLOBALS[$class]->code;
	            if (push_session_is_registered($post_var)) push_session_unregister($post_var);
          }
        }
      }
    }
