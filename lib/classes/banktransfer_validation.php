<?php
/*
  $Idä: banktransfer_validation.php 155 2005-04-04 21:05:46Z dogu $
...
// Diese function gibt die Bankinformationen aus der csv-Datei zurück*/
  function csv_query($blz) {
    $cdata = -1;
    $fp = fopen(DIR_WS_LIB . 'data/blz.csv', 'r');
...
// Diese function gibt die Bankinformationen aus der Datenbank zurück*/
  function db_query($blz) {
    if (defined(MODULE_PAYMENT_BANKTRANSFER_DATABASE_BLZ)){
      $blz_query = push_db_query("SELECT * from " . TABLE_GERMANBT_BLZ . " WHERE blz = '" . $blz . "'");
    } else {
      $blz_query = push_db_query("SELECT * from banktransfer WHERE blz = '" . $blz . "'");
    }
    if (push_db_num_rows($blz_query)){
      $data = push_db_fetch_array ($blz_query);
    }else
      $data = -1;
    return $data;
  }
...
/* ----- Die Funktionen der einzelnen Berechnungsmethoden ---- */
  ...
  function Mark24($AccountNo) {
    ...
    switch (substr($AccountNo,0,1)) {
      ...
      case 5 :
      case 6 :
        // deaktiviert, da die Postbank diese Definition nicht einhält
        //$AccountNo = Substr($AccountNo,1);
        break;
      case 9 :
...
//      $Mark73 = $this->Method06($AccountNo, 'A98765432', TRUE, 10, 11);
  ...
  function MarkC6($AccountNo) {
    $AccountNo = $this->ExpandAccount($AccountNo);
    $Help = '5499570' . substr($AccountNo,1 ,9);
...
/* ----- Ende Funktionen der einzelnen Berechnungsmethoden ---- */
