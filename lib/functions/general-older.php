...
// BOF Separate Pricing Per Customer
  ...
  $customer_group_id = '0';
     ...
     $customer_group_id = $sppc_customer_group_id;
		...
		$product_query = push_db_query("select s.specials_new_products_price from (" . $specials_without_duplicates . ") AS s where s.products_id = '" . (int)$product_id . "' and s.status and s.customers_group_id = '" . (int)$customer_group_id . "'");
...
// Calculates Tax rounding the result
  function push_calculate_tax($price, $tax) {
    ...
    return push_round($price * $tax / 100, $currencies->currencies[DEFAULT_CURRENCY]['decimal_places']);
  }
...
// Return a random value
  ...
  function push_count_customer_orders($id = '', $check_session = true, $dblink='') {
    ...
    if ($check_session == true) {
      if ( (push_session_is_registered('customer_id') == false) || ($id != $customer_id) ) {
        return 0;
      }
    }
	if($dblink==''){
	    ...
	    $orders_check_query = push_db_query("select count(*) as total from " . TABLE_ORDERS . " where customers_id = '" . (int)$id . "'", $dblink);
	}
...
// Base Price, Specials and Tax Included^
  function push_get_products_display_price($products_id, $base_price=false, $final_price=false) {
    ...
    if ($customer_group_id !='0') {
      $customer_group_price_query = push_db_query("select customers_group_price from " . TABLE_PRODUCTS_GROUPS . " where products_id = '" . $products_id . "' and customers_group_id =  '" . $customer_group_id . "'");
      if ($customer_group_price = push_db_fetch_array($customer_group_price_query)) {
        $product_check['products_price'] = $customer_group_price['customers_group_price'];
      }
    }
...
function push_make_cat_ul_list($id="fehlt", $style="")
	{
		...
		$categoriesQuery = push_db_query('SELECT c.categories_id, cd.categories_name, c.parent_id FROM ' . TABLE_CATEGORIES . ' c, ' . TABLE_CATEGORIES_DESCRIPTION . ' cd WHERE c.categories_id = cd.categories_id AND cd.language_id="' . (int)$languages_id .'" ' . $parentQuery . ' ORDER BY c.sort_order, cd.categories_name;');
		$c = 0;	
	...
	function push_make_cat_ul_branch($categories, $rootCatID, $level, $id='fehlt')
	{
		...
		{
			// is parent?
			$output .= "\n" . '<a class="' . ( push_has_category_subcategories($catID) ? CLASSNAME_FOR_PARENT : '');
			...
			if (!$level)
			{
				...
				$GLOBALS['cPath_set'][0] = $catID;
				$cPath_new = 'cPath=' . $catID;
			}
			else
			{
				$GLOBALS['cPath_set'][$level] = $catID;
			...
			$output .=  '" title="' . $catName . '" href="' . push_href_link(FILENAME_DEFAULT, $cPath_new) . '">' . $catName . '</a>';
  ...
  function get_categories_default_image($pid, $ml=''){
		/**
		 * pick the corresponding image from db and return image html
		 */
		 if($ml==TOSH)
			$cdpi=push_db_fetch_array(push_db_query("SELECT cpi.categories_image FROM categories_preview_images cpi JOIN products_to_categories p2c ON cpi.categories_id = p2c.categories_id WHERE p2c.products_id='" . $pid . "';", 'dbt'));
		else
		...
		$seite['forum'] = array (	FILENAME_FORUM);
 ...
 */
	function push_product_link_opener($produktid, $zielseite=FILENAME_PRODUCT_INFO){
		global $keywordstr,$key_categories_id;
		if (isset($_GET['manufacturers_id'])){
			echo '<a href="' . generate_url_rewritten_string(push_href_link($zielseite, 'manufacturers_id=' . $_GET['manufacturers_id'] .'&'. ($_GET['filter']? 'filter=' . $_GET['filter'] . '&' : '') . 'products_id=' . $produktid) ). '">';
		}elseif (isset($_GET['specials'])){
			if($_GET['specials']=='mhd'){
				echo '<a href="' . generate_url_rewritten_string(push_href_link($zielseite, 'specials=mhd&' . ($_GET['filter']? 'filter=' . $_GET['filter'] . '&' : '') . 'products_id=' . $produktid)) . '">';
			}else{
				echo '<a href="' . generate_url_rewritten_string(push_href_link($zielseite, 'specials=week&' . ($_GET['filter']? 'filter=' . $_GET['filter'] . '&' : '') . 'products_id=' . $produktid)) . '">';
			}
		}elseif (isset($_GET['newproducts'])){
			echo '<a href="' . generate_url_rewritten_string(push_href_link($zielseite, 'newproducts=all&' . ($_GET['filter']? 'filter=' . $_GET['filter'] . '&' : '') . 'products_id=' . $produktid)) . '">';
		}elseif (isset($_GET['keywords'])){
			echo '<a href="' . generate_url_rewritten_string(push_href_link($zielseite, 'keywords=' . $keywordstr .'&categories_id=' . $key_categories_id . ((isset($_GET['search_in_description']))? '&search_in_description=1':'') .'&products_id=' . $produktid)) . '">';
		}else{
			echo '<a href="' . generate_url_rewritten_string(push_href_link($zielseite, ($_GET['cPath']? 'cPath=' . $_GET['cPath'] . '&' : ''). ($_GET['filter']? 'filter=' . $_GET['filter'] . '&' : '') . 'products_id=' . $produktid )) . '">';
		}
	}
...
//pass through if already rewritten:
	...
	//prevent wrong links to other shops
	if(strpos($url, DIR_WS_HTTpush_CATALOG.'../')){
		//here is only a db switch needed
		//but until that:
		//Tortenshop Database
//		return $url;
		//change db for $shopbase
/*		$user_tshop = 'bkr-sho_admin';
		$pw_tshop = 'hnBBhzj!';
		$database_tshop='bkr_shop_torten';
		$t=push_db_connect('localhost', $user_tshop, $pw_tshop, $database_tshop, 'dbu');
*/
		$flag=true;
//		return $url;
	}
...
//get products_name
		...
		if($flag){
			$p=push_db_fetch_array(push_db_query($gpq,'dbt'));
		}else{
		...
		if($p['url_name']<>''){
			...
			if($flag){
				$pq=push_db_fetch_array(push_db_query("SELECT products_name FROM products_description WHERE products_id='" .(int) $product . "' ",'dbu'));
			}else{
			...
			if($pq['products_name']<>''){
				...
				if($flag){
					push_db_query("INSERT INTO url_rewriting (url_name, url_type, url_type_id) VALUES ('" . $titel . "', 'pd', '" . (int)$product . "','dbt')");
				}else{
		...
		foreach($snippets as $snippet){
			...
			if( $teile[0]=='newproducts'){
				...
				break;
					...
					if($flag){
					$tq = push_db_fetch_array(push_db_query("SELECT manufacturers_name FROM manufacturers WHERE manufacturers_id='" . $teile[1] . "'", 'dbt'));
					}else{
				...
				break;
					...
					foreach($cats as $id){ 
					if($flag){
						$tq = push_db_fetch_array(push_db_query("SELECT categories_name FROM categories_description WHERE categories_id='" . $id . "' and language_id='2'",'dbt'));
						}else{
...
*/
	...
	$revstring = $anfang . $titel .','.$ziel.','. str_replace('&redirected=true','',$parts[1]) .'.html';
